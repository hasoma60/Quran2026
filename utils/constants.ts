...
// =============================================
// Surah Start Pages (Madina Mushaf / King Fahd Complex)
// =============================================

export const SURAH_PAGES: number[] = [
...
// Arabic ordinal names for all 30 juz
export const JUZ_ARABIC_NAMES: string[] = [
  'الجُزْءُ الأَوَّلُ',
  'الجُزْءُ الثَّانِي',
  'الجُزْءُ الثَّالِثُ',
  'الجُزْءُ الرَّابِعُ',
  ...
  'الجُزْءُ التَّاسِعُ',
  'الجُزْءُ العَاشِرُ',
  'الجُزْءُ الحَادِيَ عَشَرَ',
  'الجُزْءُ الثَّانِيَ عَشَرَ',
  ...
  'الجُزْءُ الثَّلَاثُونَ',
];

// Get juz number for a given chapter id
export function getJuzForChapter(chapterId: number): number {
  for (let i = JUZ_BOUNDARIES.length - 1; i >= 0; i--) {
    const boundaryChapter = parseInt(JUZ_BOUNDARIES[i].verseKey.split(':')[0], 10);
    if (chapterId >= boundaryChapter) return JUZ_BOUNDARIES[i].juz;
  }
  return 1;
}
